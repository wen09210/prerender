...
chrome.spawn = function (options) {
  return new Promise((resolve, reject) => {
    ...
    util.log('[Chrome spawn] location:', location);
    util.log('[Chrome spawn] flags:', chromeFlags.join(' '));
    util.log('[Chrome spawn] HOME:', process.env.HOME);
    ...
    util.log('[Chrome spawn] PID:', this.chromeChild.pid);

    this.chromeChild.stdout && this.chromeChild.stdout.on('data', (data) => {
      util.log('[Chrome stdout]', data.toString());
    });

    this.chromeChild.stderr && this.chromeChild.stderr.on('data', (data) => {
      util.log('[Chrome stderr]', data.toString());
    });

    this.chromeChild.on('error', (err) => {
      util.log('[Chrome spawn error]', err.message);
    });

    this.chromeChild.on('close', (code) => {
      util.log('[Chrome exited] code:', code);
    });
